...
class SpiderEncoderV2Preproc(abstract_preproc.AbstractPreproc):
    ...
    def _augment_links_with_foreign_keys(self, sc_link, cv_link, schema, preproc_schema):
        q_tab_match = dict(sc_link.get("q_tab_match", {}))
        q_col_match = sc_link.get("q_col_match", {})
        cell_match = cv_link.get("cell_match", {})
        seed_tables_by_qid = collections.defaultdict(set)

        for key in q_tab_match.keys():
            q_id, t_id = key.split(",")
            seed_tables_by_qid[int(q_id)].add(int(t_id))

        for key in q_col_match.keys():
            q_id, c_id = key.split(",")
            table_id = preproc_schema.column_to_table.get(str(c_id))
            if table_id is not None:
                seed_tables_by_qid[int(q_id)].add(int(table_id))

        for key, match_type in cell_match.items():
        ...
        for q_id, seed_tables in seed_tables_by_qid.items():
            for table_id in list(seed_tables):
                try:
                    neighbors = schema.foreign_key_graph.neighbors(table_id)
                except Exception:
                    neighbors = []
                for neighbor_id in neighbors:
                    match_key = f"{q_id},{neighbor_id}"
                    if match_key not in q_tab_match:
                        q_tab_match[match_key] = "TPM"

        sc_link["q_tab_match"] = q_tab_match
        return sc_link
    ...
    def preprocess_item(self, item, schema, validation_info):
        ...
        if self.compute_sc_link:
            sc_link = self._augment_links_with_foreign_keys(sc_link, cv_link, schema, preproc_schema)
        return {
