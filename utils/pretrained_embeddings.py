...
class GloVe(Embedder):
    ...
    def __init__(self, kind, lemmatize=False):
        ...
        try:
            try:
                # torchtext newer versions may not expose vocab on top-level module.
                from torchtext.vocab import GloVe as TorchtextGloVe
            except Exception:
                import torchtext  # fallback for older layouts
                TorchtextGloVe = torchtext.vocab.GloVe
            self.glove = TorchtextGloVe(name=kind, cache=cache)
            self.dim = self.glove.dim
            self.vectors = self.glove.vectors
        except Exception as e:
            # Tokenization still works for preprocessing even when word vectors are unavailable.
            print(f'WARNING: failed to initialize torchtext GloVe ({e}); fallback to tokenization-only mode.', file=sys.stderr)
