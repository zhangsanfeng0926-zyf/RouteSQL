...
CANDIDATE_TRACE="results/CANDIDATE_TRACE_MODEL-${MODEL}_datasetmin_stage2_${RUN_ID}.jsonl"
...
if [[ ! -f "${STAGE1_RESULT}" ]]; then
  echo "[ERROR] Missing stage1 result file: ${STAGE1_RESULT}"
  exit 1
fi

CUR=$(wc -l < "${STAGE1_RESULT}")
log_step "RESUME_STAGE1_FROM=${CUR}"
