...
RUN_ID="${1:-V5FULL_$(date +%Y%m%d_%H%M%S)}"
...
DATA_ROOT="dataset"
PROCESS_ROOT="dataset/process_full_v5"
...
STAGE1_RESULT="results/RESULTS_MODEL-${MODEL}_spiderfull_stage1_${RUN_ID}.txt"
STAGE2_RESULT="results/RESULTS_MODEL-${MODEL}_spiderfull_stage2_${RUN_ID}.txt"
FINAL_RESULT="results/RESULTS_MODEL-${MODEL}_spiderfull_final_${RUN_ID}.txt"
MERGE_TRACE="results/RESULTS_MODEL-${MODEL}_spiderfull_merge_trace_${RUN_ID}.jsonl"
CANDIDATE_TRACE="results/CANDIDATE_TRACE_MODEL-${MODEL}_spiderfull_stage2_${RUN_ID}.jsonl"
LATEST_RESULT="results/RESULTS_MODEL-${MODEL}_spiderfull_latest.txt"
OFFICIAL_EVAL_TXT="results/spiderfull_official_eval_${RUN_ID}.txt"
OFFICIAL_EVAL_JSON="results/spiderfull_official_eval_${RUN_ID}_metrics.json"
...
run_cmd "stage1_infer" \
  "${PYTHON_BIN}" scripts/python_tools/ask_llm.py \
    ...
    --output_suffix "spiderfull_stage1_${RUN_ID}" \
...
run_cmd "stage2_infer" \
  "${PYTHON_BIN}" scripts/python_tools/ask_llm.py \
    ...
    --output_suffix "spiderfull_stage2_${RUN_ID}" \
    --result_output_dir results
...
run_cmd "merge_stage1_stage2" \
  "${PYTHON_BIN}" scripts/python_tools/merge_predictions.py \
    ...
    --pred_b_candidates "${CANDIDATE_TRACE}" \
    --db_dir "${DB_DIR}" \
    --out "${FINAL_RESULT}" \
...
log_step "PIPELINE_V5_FULL_COMPLETE run_id=${RUN_ID}"
