...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_root", type=str, default=PATH_DATA,
                        help="Root path that contains dataset folder(s), e.g. dataset or dataset/dataset_min")
    parser.add_argument("--process_root", type=str, default="dataset/process",
                        help="Output root for generated question tasks")

    args = parser.parse_args()

    # load test dataset here
    data = load_data(args.data_type, args.data_root, args.pre_test_result)
    ...
    path_generate = f"{args.process_root}/{args.data_type.upper()}-{args.split.upper()}_{prompt.name}_CTX-{args.max_ans_len}_ANS-{args.max_seq_len}"
